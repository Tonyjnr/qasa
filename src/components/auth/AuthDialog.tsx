...
export function AuthDialog({ open, onOpenChange }: AuthDialogProps) {
  ...
  // Common appearance settings for both components
  const commonAppearance = {
    ...
    elements: {
      ...
      formFieldInput:
        "bg-slate-900 border border-slate-600 text-white rounded-full h-12 focus:border-white focus:ring-0 transition-colors",
      formButtonPrimary:
        "bg-white text-slate-900 hover:bg-slate-200 rounded-full h-12 font-bold transition-colors shadow-none normal-case",
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 translate-x-[-50%] translate-y-[-50%] p-0 outline-none w-full max-w-sm">
