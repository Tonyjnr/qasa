Fix the Dashboard.tsx to be a complete resident-style dashboard with both sidebars (right one 10% smaller), and keep the ResearchOverview as a separate component.
...
const CigaretteWidget = ({ pm25 }: { pm25: number }) => {
...
const ExerciseAdvisor = ({ currentAQI, forecast }: any) => {
  const activities = [
...
const PollutantGrid = ({ pollutants }: any) => {
  const items = [
...
const ForecastList = ({ forecast }: any) => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-2">
        {forecast.slice(0, 4).map((f: any, i: number) => {
...
**For `src/pages/professional/Dashboard.tsx`** - This should be identical to the resident dashboard but with the professional sidebar (210px left, 380px right):

```tsx
// Just copy everything from src/pages/resident/Dashboard.tsx
// The only changes needed are:
// 1. Left sidebar is 210px with "Pro Research" branding
// 2. Right sidebar is 380px (down from 420px) - already using className="lg:w-[380px]"
// 3. Keep all the map, cigarette widget, exercise advisor, pollutants, forecast components
```

**For `src/pages/professional/ResearchOverview.tsx`** - Keep as a separate tab content component (already exists in your files).

The key fix is:

- Your `Dashboard.tsx` should mirror the resident dashboard completely (with map, widgets, etc.)
- The right `<Sidebar>` component should use `lg:w-[380px]` instead of `lg:w-[calc(400px_+_10%)]`
- The ResearchOverview stays as a separate component to be rendered when that tab is active.
